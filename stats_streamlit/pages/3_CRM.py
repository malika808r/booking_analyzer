...
def main():
    st.set_page_config(page_title="Booking Analyzer | Customer Insights", page_icon=None, layout="wide")
    inject_custom_css()
    user = require_login()
    render_sidebar(user)
    ...
    m3.metric("At Risk", len(df[df['Segment'] == "At Risk"]), delta_color="inverse")
    ...
    fig = px.scatter(
        ...
        color_discrete_map={"VIP": "#FFD700", "Loyal": "#00CC96", "New": "#636EFA", "At Risk": "#EF553B"},
    ...
    tab1, tab2, tab3 = st.tabs(["VIPs", "At Risk", "All Guests"])
    ...
    with tab2:
        risky = df[df['Segment'] == "At Risk"].sort_values("flakes", ascending=False)
        if not risky.empty:
